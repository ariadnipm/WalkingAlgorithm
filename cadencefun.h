...
std::vector<std::vector<double>> identify_peaks_in_cwt(
    const std::vector<double>& freqs_interp,                  // [n_freqs]
    ...
    std::vector<std::vector<double>> dp;  // θα γεμίσει παρακάτω
    ...
    const std::size_t num_rows = coefs_interp.size();
          // n_freqs
    const std::size_t num_cols = coefs_interp[0].size();   // n_time
    if (num_rows != freqs_interp.size() || num_cols == 0) {
        return dp;
    }

    // Όλες οι σειρές πρέπει να έχουν ίδιο μήκος
    ...
    // Πόσα παράθυρα 1s (μη επικαλυπτόμενα) χωράνε;
    const std::size_t num_cols2 = num_cols / static_cast<std::size_t>(fs);
    if (num_cols2 == 0) {
        return dp;
    }

    // ======= dp αρχικοποίηση σε μηδενικό πίνακα [num_rows x num_cols2] =======
    dp.assign(num_rows, std::vector<double>(num_cols2, 0.0));
    
    // ======= helper: argmin |freqs_interp - target| =======
    ...
    for (std::size_t i = 0; i < num_cols2; ++i) {
        ...
        if (x_end > num_cols) {
            break; // safety, αν για κάποιο λόγο δεν βγαίνει ακριβώς
        }
        ...
        // ======= Peaks στο window (πάνω στον άξονα των συχνοτήτων) =======
        ...
        if (locs.empty()) {
            // Δεν βρέθηκαν peaks σ’ αυτό το window
            continue;
        }

        // ======= Sort κατά φθίνουσα κορυφή, όπως np.argsort(-pks) =======
        ...
        std::sort(order.begin(), order.end(),
                  [&](std::size_t a, std::size_t b) {
                      return pks[a] > pks[b];  // descending amplitude
                  });
        ...
        // ======= Βρίσκουμε τον πρώτο peak ΜΕΣΑ στο [loc_min, loc_max] =======
        ...
        if (index_in_range_opt.has_value()) {
            ...
            // ======= max peaks κάτω και πάνω από το range =======
